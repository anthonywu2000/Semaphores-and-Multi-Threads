...
uthread_sem_t sem_lock;
//doesn't synchronize with mutex so we leave this outside
uthread_sem_t producer_add;
uthread_sem_t consumer_retrieve;
...
void* producer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_sem_wait(producer_add);
    uthread_sem_wait(sem_lock);
    items += 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;
    uthread_sem_signal(sem_lock);
    uthread_sem_signal(consumer_retrieve);
  }
  return NULL;
}
...
void* consumer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_sem_wait(consumer_retrieve);
    uthread_sem_wait(sem_lock);
    items -= 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;
    uthread_sem_signal(sem_lock);
    uthread_sem_signal(producer_add);
  }
  return NULL;
}

int main (int argc, char** argv) {
  sem_lock = uthread_sem_create(1);
  producer_add = uthread_sem_create(MAX_ITEMS);
  consumer_retrieve = uthread_sem_create(0);
  ...
  uthread_sem_destroy(sem_lock);
  uthread_sem_destroy(producer_add);
  uthread_sem_destroy(consumer_retrieve);
}
